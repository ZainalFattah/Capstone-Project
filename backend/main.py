...
async def lifespan(app: FastAPI):
    ...
    CHROMA_CLIENT = chromadb.PersistentClient(path="./DB_PAPER")
    ...
    LLM = Llama(model_path=model_path, n_ctx=4096, n_gpu_layers=45, temperature=0.5, top_p=0.5, verbose=True)
...
@app.get("/")
def root(): return {"message": "RAG API is running."}
...
def get_history():
    ...
    try:
        ...
        for row in rows:
            ...
            if row['chat_logs']:
                try:
                    row['chat_logs'] = json.loads(row['chat_logs'])
                except:
                    row['chat_logs'] = []
        return rows
    except Error as e:
    ...
    try:
        ...
        try:
            ...
            if conn:
                ...
                val = (document_id, file.filename, json.dumps(final_summary), json.dumps([]))
                cursor.execute(sql, val)
                conn.commit()
                cursor.close()
                conn.close()
                print("✅ Saved to history DB.")
        except Exception as db_e:
            print(f"⚠️ Failed to save to DB: {db_e}")
...
async def question_answering(request: QARequest):
    try:
        ...
        # --- SAVE CHAT LOG TO DB ---
        active_document_id = request.document_id  # Track the ID being used
        ...
        try:
            ...
            if conn:
                ...
                if request.document_id:
                    # Jika ada document_id, update history yang sudah ada
                    cursor.execute("SELECT chat_logs FROM history WHERE id = %s", (request.document_id,))
                    row = cursor.fetchone()
                    
                    current_logs = []
                    if row and row['chat_logs']:
                        try:
                            current_logs = json.loads(row['chat_logs'])
                        except:
                            current_logs = []
                    ...
                    current_logs.append(new_chat)
                    
                    # Update DB
                    cursor.execute("UPDATE history SET chat_logs = %s WHERE id = %s", (json.dumps(current_logs), request.document_id))
                    conn.commit()
                    print(f"✅ Chat log updated for {request.document_id}. Rows affected: {cursor.rowcount}")
                else:
                    # Jika tidak ada document_id, buat entry baru untuk Q&A standalone
                    new_id = str(uuid.uuid4())
                    active_document_id = new_id  # Set the new ID to return
                    ...
                    # Buat summary kosong untuk Q&A standalone
                    empty_summary = {
                        "Tujuan Penelitian": "",
                        "Metode": "",
                        "Hasil Utama": "",
                    ...
                    sql = "INSERT INTO history (id, filename, summary, chat_logs, created_at) VALUES (%s, %s, %s, %s, NOW())"
                    val = (new_id, "Q&A Session", json.dumps(empty_summary), json.dumps([new_chat]))
                    cursor.execute(sql, val)
                    conn.commit()
                    print(f"✅ New Q&A session created with ID: {new_id}")
                
                cursor.close()
                conn.close()
        except Exception as db_e:
            print(f"⚠️ Failed to save chat log: {db_e}")

        return {"answer": answer, "document_id": active_document_id}
